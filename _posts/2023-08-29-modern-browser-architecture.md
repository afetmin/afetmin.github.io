...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312133806-tjwh7ad.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312133824-h0sn3u5.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312133837-c8k4k3b.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312134331-3w1t1ae.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312134436-1adez7e.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312134530-4bykzl0.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312134903-1jkqnrg.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312134927-xmd3rki.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312135328-te7ksoy.png)
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312135803-xkzmz1m.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230312135933-smj806a.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319153441-31vjdda.png)​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319153744-6vv264q.png "浏览器进程告诉新的渲染进程去渲染新的页面并且告诉当前的渲染进程进行收尾工作")
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319154027-e632owr.png "网络线程会在收到导航任务后寻找有没有对应的service worker")​

​![image](/assets/img/posts/modern-browser-architecture/image-20230319154105-btbz0mt.png "UI线程会启动一个渲染进程来运行找到的service worker代码，代码具体是由渲染进程里面的工作线程（worker thread）执行")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319154244-zhy5ua4.png "UI线程在启动一个渲染进程去运行service worker代码的同时会并行发送网络请求")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319154659-kw7jd2v.png "渲染进程里面有：一个主线程（main thread），几个工作线程（worker threads），一个合成线程（compositor thread）以及一个光栅线程（raster thread）")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319154933-woxzy57.png "主线程会解析HTML内容并且构建出DOM树")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319155519-mxelx7a.png "主线程解析CSS来确定每个元素的样式")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319155728-zpsu03i.png "主线程会遍历每个DOM tree节点的计算样式信息来生成一棵布局树")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319160030-3lh6wcl.png "单纯按照HTML布局的顺序绘制页面的元素是错误的，因为元素的z-index元素没有被考虑到")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319160057-3urea4i.png "主线程遍历布局树来生成绘画记录")​
...
<video controls="controls" src="/assets/media/posts/modern-browser-architecture/d7zOpwpNIXIoVnoZCtI9-20230319160219-xxe6fk0.mp4" data-src="/assets/media/posts/modern-browser-architecture/d7zOpwpNIXIoVnoZCtI9-20230319160219-xxe6fk0.mp4"></video>
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319160317-cdcke9s.png "时间轴上的动画帧")​

即使您的渲染操作跟上屏幕刷新，这些计算也在主线程上运行，这意味着当您的应用程序运行 JavaScript 时它可能会被阻塞。

​![image](/assets/img/posts/modern-browser-architecture/image-20230319160347-cfhdaja.png "时间轴上的动画帧，但其中一帧被 JavaScript 阻塞")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319160411-x3cu2fw.png "在动画帧上运行一小段JavaScript代码")​
...
<video controls="controls" src="/assets/media/posts/modern-browser-architecture/AiIny83Lk4rTzsM8bxSn-20230319160944-s4624os.mp4" data-src="/assets/media/posts/modern-browser-architecture/AiIny83Lk4rTzsM8bxSn-20230319160944-s4624os.mp4"></video>
...
<video controls="controls" src="/assets/media/posts/modern-browser-architecture/Aggd8YLFPckZrBjEj74H-20230319161204-jvnh77i.mp4" data-src="/assets/media/posts/modern-browser-architecture/Aggd8YLFPckZrBjEj74H-20230319161204-jvnh77i.mp4"></video>
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319161239-afiqzs9.png "主线程遍历布局树来生成层次树")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319161506-eqfwdch.png "光栅线程创建切片位图并发送到 GPU")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319161750-ist00f7.png "合成线程构建出合成帧，合成帧会被发送给浏览器进程然后再发送给GPU")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319162855-53s7ztj.png "点击事件从浏览器进程路由到渲染进程")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319163059-bwu8ou0.png "非快速滚动区域有用户事件发生时的示意图")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319163253-7ffs4bl.png "当整个页面都是非快速滚动区域时页面的事件处理示意图")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319163754-0xaztgz.png "部分页面固定为水平滚动的网页")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319163931-uh70rnd.png "主线程通过遍历绘画记录来确定在x，y坐标上的是哪个对象")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319164401-5hgux1w.png "事件淹没了屏幕刷新的时间轴，导致页面很卡顿")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319164437-37sper5.png "和之前相同的事件轴，可是这次事件被合并并延迟调度了")​
...
​![image](/assets/img/posts/modern-browser-architecture/image-20230319164547-gn34roi.png "左边是顺畅的触摸手势，右边是事件合成后不那么连续的手势")​
